...
@section('title', 'All Customers | E-Commerce Dashboard')
@section('page_title', 'All Customers')
@section('page_subtitle', 'Browse and manage your customer database.')
    ...
    @if($customers->isEmpty())
        <div class="flex-grow flex items-center justify-center">
            <div class="border border-dashed border-slate-800 rounded-3xl p-12 text-center max-w-md w-full bg-slate-900/20 backdrop-blur-sm">
                ...
                <h2 class="text-xl font-bold text-white mb-1.5">No Customers Found</h2>
                <p class="text-sm text-slate-400">Database customers table is empty. Use seeders to populate dummy profiles.</p>
            </div>
        </div>
    @else
                    ...
                    <tbody class="divide-y divide-slate-800/60 pl-0">
                        @foreach($customers as $customer)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-block text-xs font-semibold px-3 py-1 rounded-full capitalize {{ strtolower($customer->city) == 'cairo' ? 'bg-violet-500/10 border border-violet-500/30 text-violet-300 shadow-[0_0_10px_rgba(139,92,246,0.1)]' : 'bg-slate-800/80 border border-slate-700/60 text-slate-400' }}">
                                        {{ $customer->city }}
                                    </span>
                                </td>
                            </tr>
